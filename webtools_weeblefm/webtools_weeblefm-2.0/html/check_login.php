...
*/
  ...
  {
    echo "<span name='error' class='error'>Missing username or password.</span>";
    exit;
  }
  ...
  if ( isset ($login_server) && compat_array_key_exists ($login_server, $ftp_Servers) ) {
    ...
    echo "<span name='error' class='error'>Server could not be found.</span>";
    exit;
  }
  ...
  if ( $result == 0 )
  {
    echo "<span name='error' class='error'>Incorrect username or password.</span>";
    exit;
  }
  ...
  if ( $ftp_disable_mcrypt ) {
    ...
    echo "<span name='error' class='error'>Encryption issue.</span>";
    exit;
  } 
  ...
  if ( $ftp_Passive_Mode ) {
    ...
    if ( $result == 0 ) {
      $sess_Data["warn"] = "Error, unable to set FTP connection to passive mode.";
  ...
  // Magic echo to let us know we can redirect to ftp.php using javascript
  echo 'SID='.session_id();
  exit;
?>
