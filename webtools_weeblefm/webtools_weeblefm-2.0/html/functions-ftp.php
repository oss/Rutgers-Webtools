...
//  1099511627776 = 1TB
  ...
  function display_row( $data, $class, $enabled )
  {
    ...
    if ( $data[6] == "D" )
      $display .= "<td width=\"95%\"><a href=\"crossover.php?SID=$SID&submit=CD&CHDIR=" . urlencode($data[8]) . "\">$data[0]</a></td><td>&nbsp;</td>\n";
    else {
      $display .= "<td width=\"95%\"><a href=\"crossover.php?SID=$SID&submit=DOWNLOAD&FILE=" . urlencode($data[0]) . "\">$data[0]</a></td>\n";
      $display .= "<td>[<a target=\"_blank\" href=\"viewer.php?SID=$SID&Filename=" . urlencode ($data[0]) . "\">View</a>]&nbsp;</td>\n";
    }
